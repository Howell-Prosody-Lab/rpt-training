...
#################################################
general_base = "E:/Shared drives/Prosody/Boston_University_Radio_Speech_Corpus/bu_radio/data/"
#OR
csv_base = "C:/Users/shein/Downloads/"
#################################################
current_date = datetime.date.today().strftime("%Y-%m-%d")
csv_output_folder = os.path.join(csv_base, f"RPT_data_{current_date}")
os.makedirs(csv_output_folder, exist_ok = True)
intensity_folder = os.path.join(csv_output_folder, "Pitch")
...
wav_end = ".wav"
#OR
text_mid = "aligner_output/"
#################################################
text_end = ".TextGrid"
#################################################

speakers = ['f1a', 'f2b', 'f3a', 'm1b', 'm2b', 'm3b']
folders = ['j', 'p', 'r', 't']
...
for speaker in speakers:
    for folder in folders:
        ...
        for i in range(1, 8):
            try:
                csv_file = os.path.join(intensity_folder, f"{speaker}{folder}{general_name}{i}{csv_end}")
                Textgrid_path = general_base + speaker + general_mid + folder + general_direct + text_mid + speaker + folder + general_name + str(i) + text_end
                Wav_file_path = general_base + speaker + general_mid + folder + general_direct + speaker + folder + general_name + str(i) + wav_end
                ton_file = general_base + speaker + general_mid + folder + general_direct + speaker + folder + general_name + str(i) + ton_end
            
                Wav_file = parselmouth.Sound(Wav_file_path)
            
                data, error, error_arr = fpi.iterateTextGridforPitch(Textgrid_path, tier_name, Wav_file)
